...
import frc.robot.subsystems.MusicState
import frc.robot.subsystems.OperatorInterfaceState
import frc.robot.subsystems.SwerveState
import frc.robot.subsystems.calibrateIfRequested
import frc.robot.subsystems.getDriveMotorSpeeds
import frc.robot.subsystems.getElevatorPower
import frc.robot.subsystems.incrementTalonsInitialized
import frc.robot.subsystems.logError
import frc.robot.subsystems.playSong
import frc.robot.subsystems.recalibrateIfRequested
import frc.robot.subsystems.resetLoaded
import frc.robot.subsystems.updateDriver
import frc.robot.subsystems.updateHeading
import frc.robot.subsystems.updateManipulator
import frc.robot.subsystems.updatePosition
import frc.robot.subsystems.updateWithPodPosition
...
object ControlSubsystem {
    ...
    fun update(msg: Message, model: Model): Pair<Model, List<Effect<ControlMessage>>> {
        return when (msg) {
    ...
    private fun handleRobotState(state: RunningRobotState, model: Model): Pair<Model, List<Effect<ControlMessage>>> {
    ...
    private fun handleTeleop(model: Model): Pair<Model, List<Effect<ControlMessage>>> {
        ...
        val swerveEffects: List<Effect<ControlMessage>> = driveMotorSpeeds.flatMap { entry ->
    ...
    private fun stopSongsForMotors(motors: Set<Kraken>, model: Model): Pair<MusicState, List<Effect<ControlMessage>>> {
                ...
                model.orchestras[motor]?.let { orchestra ->
                    Effect.StopSong(
                            ...
                            message = { result ->
                                ...
                                ControlMessage(Message.LogError(
    ...
    private fun handleRobotStateChanged(msg: Message.RobotStateChanged, model: Model): Pair<Model, List<Effect<ControlMessage>>> {
        val effects =
                                    ...
                                    model.orchestras[motor]?.let { orchestra ->
                                        listOf(
                                                Effect.StopSong(
                                                        ...
                                                        message = { result ->
                                                            val error = if (result is Result.Error) Maybe.Some(result.value) else Maybe.None
                                                            ControlMessage(Message.RestartSong(
                                                                    motor = entry.toPair(),
                                                                    error = error
                                                            ))
                                                        }
    ...
    private fun handleInitSong(msg: Message.InitSong, model: Model): Pair<Model, List<Effect<ControlMessage>>> {
            ...
            is Result.Success -> {
                ...
                listOf(model.tokens.get(motor).unwrap(
                        ...
                        fn = { token -> Effect.LoadSong(
                                ...
                                message = { result ->
                                    val error = if (result is Result.Error) Maybe.Some(result.value) else Maybe.None
                                    ControlMessage(Message.ProcessLoadedSong(msg.motor, error))
                                }
                        )}
                ))
            }
    ...
    private fun handleProcessLoadedSong(msg: Message.ProcessLoadedSong, model: Model): Pair<Model, List<Effect<ControlMessage>>> {
        val logErrorEffect: List<Effect<ControlMessage>> = logError(
                        ...
                        fn = { _ ->
                            model.orchestras[motor]?.let { orchestra ->
                                Effect.PlaySong(
                                        ...
                                        message = { result ->
                                            val error = if (result is Result.Error) Maybe.Some(result.value) else Maybe.None
                                            ControlMessage(Message.ActivateSong(entry.toPair(), error))
                                        }
                                )
    ...
    private fun handleActivateSong(msg: Message.ActivateSong, model: Model): Pair<Model, List<Effect<ControlMessage>>> {
    ...
    private fun handleInitDevice(msg: Message.InitDevice, model: Model): Pair<Model, List<Effect<ControlMessage>>> {
        val mismatchError: List<Effect<ControlMessage>> = listOf(Effect.Log("Could not find CAN device" +
    ...
    private fun handleRestartSong(msg: Message.RestartSong, model: Model): Pair<Model, List<Effect<ControlMessage>>> {
        ...
        return model to (log + Effect.ReadFile(
                ...
                message = { result -> ControlMessage(Message.InitSong(msg.motor, result)) }
        ))
    }

    fun subscriptions(model: Model): List<Subscription<ControlMessage>> {
        val pigeon = CanDevice.Pigeon.CentralPigeon
        return listOf(
                model.hidInputs[0]?.let {
                    listOf(Subscription.HidPortValue(
                            token = it,
                            message = { controllerState -> ControlMessage(Message.DriverControllerState(controllerState)) },
                    ))
                } ?: emptyList(),
                model.hidInputs[1]?.let {
                    listOf(Subscription.HidPortValue(
                            token = it,
                            message = { controllerState -> ControlMessage(Message.ManipulatorControllerState(controllerState)) },
                    ))
                } ?: emptyList(),
                listOf(Subscription.RobotStateChanged(
                        message = { prevState, newState -> ControlMessage(Message.RobotStateChanged(prevState, newState)) }
                )),
                listOf(Subscription.RobotState(
                        message = { state -> ControlMessage(Message.RobotState(state)) }
                )),
                model.tokens.get(pigeon).unwrap(
                        ...
                        fn = { token ->
                            listOf(Subscription.PigeonValue(
                                    ...
                                    message = { value -> ControlMessage(Message.PigeonValue(pigeon, value)) }
                            ))
                        }
                )
                        ...
                        listOf(Subscription.CANcoderValue(
                                ...
                                message = { pos ->
                                    if (elevatorDevices.contains(encoder))
                                        ControlMessage(Message.UpdateElevatorPosition(encoder, pos))
                                    else getSwervePod(encoder).unwrap(
                                            default = ControlMessage(Message.Log("fetching corresponding swerve pod to " +
                                                    "encoder: ${encoder.name}")),
                                            fn = { pod -> ControlMessage(Message.UpdateSwervePodPosition(pod, pos)) }
                                    )
    ...
    fun init(args: List<String>): Pair<Model, List<Effect<ControlMessage>>> {
        CANBus()
        ...
        val initDevices: List<Effect<ControlMessage>> =
                listOf(
                        ...
                        .map { (device, type) ->
                            Effect.InitCanDevice(
                                    type = type,
                                    id = device.id,
                                    message = { dev, id, token -> ControlMessage(Message.InitDevice(dev, id, token)) }
                            )
                        }
    ...
    private fun getSwervePod(encoder: CanDevice.Encoder) : Maybe<SwervePod> {
        return SwervePod.entries.associateBy { it.encoder }[encoder]?.let {
            Maybe.Some(it)
        } ?: Maybe.None
    }

    private fun getDeviceById(id: Int) : Maybe<CanDevice<*>> =
